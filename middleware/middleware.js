...
exports.hasRoundThreeStarted = (catchAsync(async (req, res, next) => {
    const team = await Team.find({});
    if (!(team[0].hasRoundThreeStarted)) {
        return next(
            new AppError("Round Three has not Started", 412, errorCodes.ROUND_THREE_NOT_STARTED)
        );
    }
    next();
}));